...
const getWorker = async () => {
  ...
  const handleCapture = () => {
    ...
    // Focused crop box targeting center odometer reel/window
    const cropX = Math.floor(vWidth * 0.10);
    const cropY = Math.floor(vHeight * 0.22);
    const cropW = Math.floor(vWidth * 0.80);
    ...
    if ('filter' in cropCtx) {
      cropCtx.filter = 'contrast(135%) brightness(105%)';
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="📷 Scan Vehicle Odometer" maxWidth="max-w-lg" zIndex={80}>
      <div className="space-y-3">
        {error && (
          <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl flex items-center space-x-2 text-xs text-amber-800">
        ...
        {!capturedImage && (
          <div className="space-y-3">
            <div className="relative bg-slate-900 rounded-2xl overflow-hidden aspect-[4/3] flex items-center justify-center border border-slate-800 shadow-inner min-h-[200px] max-h-[260px]">
              <video
                ref={videoRef}
                autoPlay
                playsInline
              ...
              {cameraActive && (
                <>
                  <div className="absolute inset-x-6 top-1/4 bottom-1/4 border-2 border-dashed border-amber-400/80 rounded-xl flex items-center justify-center bg-amber-400/10 backdrop-blur-[1px] pointer-events-none">
        ...
        {capturedImage && !processing && (
          <div className="space-y-3">
            <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-900 max-h-32 sm:max-h-40 flex items-center justify-center">
              <img src={capturedImage} alt="Captured Odometer" className="max-h-32 sm:max-h-40 object-contain" />
              <button
                onClick={handleRetake}
                className="absolute top-2 right-2 p-1.5 bg-slate-900/80 hover:bg-slate-900 text-white rounded-lg text-xs font-medium flex items-center space-x-1 backdrop-blur-xs cursor-pointer"
              >
            ...
            <div className="p-3 sm:p-4 bg-indigo-50/60 border border-indigo-100 rounded-2xl space-y-2.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3.5 py-2 bg-white border border-indigo-200 rounded-xl text-base font-bold text-slate-800 tracking-wider focus:ring-2 focus:ring-indigo-500 focus:outline-hidden"
                />
                <FaEdit className="absolute right-3.5 top-3 text-slate-400 w-3.5 h-3.5 pointer-events-none" />
              </div>
              ...
              {candidates.length > 0 && (
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 max-h-20 sm:max-h-24 overflow-y-auto p-1.5 bg-white border border-indigo-150 rounded-xl shadow-xs">
                    {candidates.map((cand, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-2 py-0.5 rounded-lg text-xs font-mono font-bold transition-all cursor-pointer flex items-center space-x-1 ${
                            isSelected
              ...
              <p className="text-[10px] text-slate-500 leading-snug">
                Check if the extracted number matches your meter display. Tap any detected number above or type to correct.
              </p>
              ...
              {editedValue.length >= 6 && (
                <button
                  ...
                  className="w-full text-left text-[10px] font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-200 px-2.5 py-1 rounded-xl flex items-center justify-between cursor-pointer transition-colors"
                >
                  <span>🔴 Is 6th digit RED (Tenths)? Click to trim 6th digit</span>
                  <span className="bg-amber-200/80 px-1.5 py-0.5 rounded text-[10px] font-mono">Use {editedValue.slice(0, -1)} KM</span>
                </button>
              )}
            </div>
            ...
            <div className="flex justify-end space-x-2.5 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-indigo-600 text-white rounded-xl text-xs font-semibold hover:bg-indigo-700 flex items-center space-x-1.5 shadow-md shadow-indigo-100 cursor-pointer"
              >
